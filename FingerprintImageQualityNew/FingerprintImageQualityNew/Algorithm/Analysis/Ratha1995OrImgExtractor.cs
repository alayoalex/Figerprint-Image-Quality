...
using FingerprintImageQualityNew.ImageProcessing.ConvolutionFilters;
...
namespace FingerprintImageQualityNew.Algorithm.Analysis
{
    ...
    public class Ratha1995OrImgExtractor
    {
        ...
        public OrientationImage ExtractOrientationImage(Bitmap image)
        {
            ...
            //OrientationCoherenceMap coherenceMap = new OrientationCoherenceMap(width, height);
            ...
            for (int row = 0; row < height; row++)
                for (int col = 0; col < width; col++)
                {
                    ...
                    else
                    {
                        ...
                        //coherenceMap.Block[row, col] = Math.Sqrt((Math.Pow(Gxx - Gyy, 2)) + 4 * Math.Pow(Gxy, 2)) / (Gxx + Gyy);
                    }
                }
        ...
        public OrientationCoherenceMap ComputingCoherenceMap(Bitmap image) 
        {
            ImageMatrix matrix = new ImageMatrix(image);
            ...
            OrientationImage oi = new OrientationImage(width, height, BlockSize);
            OrientationCoherenceMap coherenceMap = new OrientationCoherenceMap(height, width);
            ...
            for (int row = 0; row < height; row++)
                for (int col = 0; col < width; col++)
                    if (oi[row, col] == OrientationImage.Null)
                        coherenceMap.Block[row, col] = OrientationImage.Null;
            return coherenceMap;
        }
        ...
        //TODO: bSize puede cambiar para imágenes más pequeñas.
